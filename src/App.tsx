...
/**
 * 智能資訊牆 V10.1 Compact Elite
 * 優化重點：
 * 1. 導覽列高度縮減，騰出可視空間
 * 2. 導入「網格高密度」佈局，手機版雙欄顯示
 * 3. 縮減卡片內距、字體、圖片高度，提升資訊密度
 * 4. 強化 TypeScript 型別嚴謹度
 */
      ...
      <header className="bg-white/80 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-200/50">
        <div className="max-w-[1920px] mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white text-lg shadow-md shadow-indigo-100">📋</div>
            <h1 className="text-lg font-black tracking-tighter text-slate-800">智能資訊牆</h1>
          </div>
          {profile && (
            <div className="flex items-center gap-2 bg-white/50 p-1 pr-3 rounded-xl border border-slate-200/60 shadow-sm">
              <img src={profile.pictureUrl} className="w-6 h-6 rounded-lg object-cover" alt="User" />
        ...
        <div className="max-w-[1920px] mx-auto px-4 pb-3 flex gap-1.5 overflow-x-auto no-scrollbar">
          {['全部', '郵件', '檔案', '連結'].map(t => (
            <button 
              key={t}
              onClick={() => setFilter(t)}
              className={`px-4 py-1.5 rounded-xl text-[11px] font-black whitespace-nowrap transition-all duration-200 ${
                filter === t 
                ? 'bg-slate-900 text-white shadow-md' 
                : 'bg-white text-slate-400 border border-slate-200/60 hover:text-slate-600'
              }`}
            >
              {t}
            </button>
      ...
      <main className="max-w-[1920px] mx-auto p-3 sm:p-6 lg:p-8">
        {isLoading ? (
          <div className="py-40 text-center flex flex-col items-center gap-4">
            ...
            <p className="text-[11px] font-black text-slate-300 tracking-widest uppercase">載入雲端數據</p>
          </div>
            ...
            {filteredItems.map(item => {
              ...
              return (
                <div 
                  ...
                  className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-pointer flex flex-col"
                >
                  {item.type === 'file' && thumb && (
                    <div className="w-full h-24 sm:h-28 bg-slate-50 overflow-hidden">
                      <img src={thumb} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="預覽" />
                    </div>
                  )}
                  
                  <div className="p-3 sm:p-4 flex flex-col">
                    <div className="flex justify-between items-center mb-2 pb-2 border-b border-slate-50">
                      <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[8px] font-black uppercase tracking-tighter ${item.color || 'bg-slate-100 text-slate-500'}`}>
                        <span>{item.icon}</span>
                        <span>{item.type === 'email' ? '郵件' : item.type === 'file' ? '檔案' : '連結'}</span>
                      </div>
                      <div className="text-[8px] font-bold text-slate-300">
                        {item.date ? item.date.split('T')[0].slice(5) : ''}
                      </div>
                    </div>
                    
                    <h2 className="text-xs sm:text-sm font-black text-slate-800 leading-tight mb-1 line-clamp-2 min-h-[32px] group-hover:text-indigo-600 transition-colors">
                      {item.title}
                    </h2>
                    
                    <div className="bg-slate-50/80 rounded-lg p-2 text-[10px] text-slate-500 leading-snug font-medium line-clamp-3 min-h-[48px] border border-slate-100/30">
                      {item.content}
                    </div>
                    
                    <div className="mt-2 flex justify-end">
                       <span className="text-[8px] font-black text-slate-200 group-hover:text-indigo-500 transition-colors uppercase tracking-widest">Detail ↗</span>
                    </div>
                  </div>
                </div>
              );
